...
from .WTConv import E_WTConv2d as WTConv2d


class WTConvLSTMCell(nn.Module):

    def __init__(self, in_channel, num_hidden, height, width, filter_size, stride=1, layer_norm=True):
        super(WTConvLSTMCell, self).__init__()
        ...
        if layer_norm:
            self.conv_x = nn.Sequential(
                WTConv2d(in_channel, num_hidden * 4, kernel_size=filter_size, ),
                nn.LayerNorm([num_hidden * 4, height, width])
            )
            self.conv_h = nn.Sequential(
                WTConv2d(num_hidden, num_hidden * 4, kernel_size=filter_size, ),
                nn.LayerNorm([num_hidden * 4, height, width])
            )
            self.conv_o = nn.Sequential(
                WTConv2d(num_hidden * 2, num_hidden, kernel_size=filter_size, ),
